...
Notes:
- Uses the v=0 length-force data (total force) generated by 01_extract_opensim_data.py.
...
# Add scripts directory to path to import from sibling module
sys.path.insert(0, str(Path(__file__).parent))

# Import from the fitting script (sibling module)
import importlib.util
spec = importlib.util.spec_from_file_location("fit_module", Path(__file__).parent / "02_fit_mujoco_params.py")
fit_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(fit_module)

CompliantTendonParams = fit_module.CompliantTendonParams
create_model = fit_module.create_model
compute_forces_at_velocity = fit_module.compute_forces_at_velocity
load_length_force_sim = fit_module.load_length_force_sim
